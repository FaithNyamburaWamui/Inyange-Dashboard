...
const EditMaterialModal = ({ materialData, onClose, onMaterialUpdated }: EditMaterialModalProps) => {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(
    materialData && typeof materialData.image === 'string' ? materialData.image : null
  );
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setImagePreview(materialData && typeof materialData.image === 'string' ? materialData.image : null);
    }
  };
  ...
  const onSubmit: SubmitHandler<MaterialData> = async (data) => {
    ...
    try {
      const response = await editMaterial({ ...data, material_id: materialData.material_id }); 
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          <div className="flex justify-end mt-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`px-4 py-2 bg-[#F8B612] text-[#263C5A] font-bold rounded-lg hover:bg-[#E6A300] transition-colors duration-300 ${loading ? 'opacity-50 cursor-notAllowed' : ''}`}
            >
